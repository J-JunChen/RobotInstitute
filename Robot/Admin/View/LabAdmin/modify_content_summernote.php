...
<body>
    ...
    <?php
       ...
       $json_string = session('json_string');

        $content_table_whole = json_decode($json_string,TRUE);

                
          foreach ($content_table_whole as $key => $value) {

            echo '<input type="text" name="title" id="title" maxlength="30" placeholder="标题：不能为空（不能超过30字）" value = '.$value['title'].'>';
          }
      ?>
    ...
    <div id="content">
          ...
          <div id="content-title">
              ...
              <div>     
                    ...
                    <?php
                         foreach ($content_table_whole as $key => $value) {
                        // echo '<a href="modify?id='.$value['id'].'"<button  type="button" class="btn btn-info" id="modify">&nbsp;修改&nbsp;</button></a>';
                          echo '<button type="button" class="btn btn-info" id="modify">&nbsp;修改&nbsp;</button>';
                      }
                    ?>
