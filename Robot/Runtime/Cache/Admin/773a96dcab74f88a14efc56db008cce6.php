 ...
 -->
     ...
     <main class="app-content">
      <div class="app-title">
        <div>
             <?php
 echo '<h1><i class="fa fa fa-heart"> '.session('catalog').'</i></h1>'; ?>
            ...
            <div class="tile-body">
              <table class="table table-hover table-bordered" id="contentTable">
                ...
                <tbody>
                  ...
                  <?php
 use Think\Model; $table = new Model(session('catalog_table')); $data = $table->order('id desc')->select(); foreach ($data as $key => $value) { echo "<tr><td><a id='review' href='review_content?id=".$value['id']."'>". $value['title']."</a></td> <td>".date('Y-m-d',$value['date'])."</td><td>".$value['user_name']."</td><td>".$value['user_type']."</td><td><a id='review' href='modify_content?id=".$value['id']."'>修改</a> | <span><a href='javascript:void(0)' style='color:red;'data ='".$value['id']."'>删除</a></span></td></tr>"; } ?>
                 </tbody>
              </table>
            </div>
          </div>
    ...
    <script src="/RobotInstitute/Public/js/Admin/working_content.js"></script>
