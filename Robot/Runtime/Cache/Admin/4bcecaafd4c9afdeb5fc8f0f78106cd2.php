...
<body>
    ...
    <?php
 use Think\Model; $table = new Model(session('catalog_table')); $data = $table->select(); foreach ($data as $key => $value) { echo '<input type="text" name="title" id="title" maxlength="30" placeholder="标题：不能为空（不能超过30字）" value = '.$value['title'].'>'; } ?>
    ...
    <div id="content">
         ...
         <div id="summernote">
              <?php
 foreach ($data as $key => $value) { echo $value['content']; } ?>
         </div>
      </div>
...
$("#modify").click(function () { //点击修改按钮，
    ...
    var id = <?php  foreach ($data as $key => $value) echo $value['id']; ?>;
      ...
      alert('标题不能为空');
        ...
        $.ajax({
          ...
          success: function (msg) {
            ...
            window.parent.location.href = "institute_overview?title="+msg.catalog+"&content="+msg.catalog_table;//父页跳转
          },
